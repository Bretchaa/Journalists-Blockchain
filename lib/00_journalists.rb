...
def position_epenser(journalists)
    puts "La position dans l'array de la personne @epenser est : #{journalists.index {|x| x == "@epenser"}}"
end 

def repartition(journalists)
    number_character=0
    while number_character <=12
        puts "Le nombre de handles avec #{number_character} caractères est #{journalists.count {|x| x.length == number_character}}"
        number_character += 1
     end
end

puts "Bonjour et bienvenue dans notre super application!"
puts "A quelle question veux tu une réponse?"
puts "1_ Combien y a-t-il de handle dans cette array ?"
puts "2_ Quel est le handle le plus court?"
puts "3_ Combien y-a-t'il de handle contenant 5 caractères?"
puts "4_ Combien commencent par une majuscule?"
puts "5_ Trie la liste de handle par ordre alphabétique."
puts "6_ Trie la liste de handle par taille des handle"
puts "7 _ Quelle est la position dans l'array de la personne @epenser ?"
puts "8_ Sors-moi une répartition des handle par taille de ces derniers"
...
if input ==1
    ...
    alphabetical_array(journalists)
elsif input == 6 
...
else print "Ca correspond à aucune question :( "
end
